...
'''Använder här pyscbwrapper för att hämta befolkningsdata från SCB-databasen för senare användning av beräkning av CNI'''
sys.stdout.reconfigure(encoding='utf-8')

# Funktion för att hämta specificerad tabelldata. Returner tabellen i json-format och extraherar ort, län och respektive kod
def get_tables(a, b, c, d=''):
    ...
    orter = [x for x in regioner if x not in lan][1:]       # Sållar bort länen från alla områden
    ortkoder = [x for x in koder if len(x) != 2]            # Ortkoder är längre än 2 siffror
...
# I det första fallet är en datapunkt varje ålder mellan 65-100
def get_data(scb_data):
    scb_data = scb_data.get_data()['data']
    d = collections.defaultdict(list)
    for i in range(len(scb_data)):              #   Loopsar så många gånger det finns datapunkter per län
        d[scb_data[i]['key'][0][:2]].append(scb_data[i]['values'][0])   # Lägger till antal personer per datapunkt i respektive län
    varden = []
    for i in d:
        varden.append([np.sum([int(i) for i in d[i]])])
    return varden                                           #   Returnerar en lista med alla antal summerat per län

# Ensamboenede 65+ (se sista raden per paragraf
...
time.sleep(10)      # SCB stoppar förfrågningarna om man ställer för många under kort tid.
...
# Läser in arbetslösa samt utlandsfödda i separata filer
arbetslosa = pd.read_pickle('arbetslosa.pickle')
utlandsfodda = pd.read_pickle('utlandsfodda.pickle')


# Lägger ihop all data till en dataframe
